...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const handleGlobalClick = () => setActiveCardId(null);
      ...
      <div className="hero__container">
        <div className={`hero__network ${isMounted ? 'hero__network--mounted' : ''}`}>
          <button
            ...
            type="button"
            onClick={(e) => { e.stopPropagation(); setActiveCardId(activeCardId === 'center' ? null : 'center'); }}
            onMouseEnter={() => setHoveredCardId('center')}
            onMouseLeave={() => setHoveredCardId(null)}
            aria-label="CoreShift HR Platform Core"
          >
            <div className="hero__center-icon"><HiCheck /></div>
          ...
          {floatingCards.map((card) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={(e) => { e.stopPropagation(); setActiveCardId(activeCardId === card.id ? null : card.id); }}
              ...
              >
                {'src' in card ? (
                  <div className="hero__avatar-wrapper">
                    <img src={card.src} alt={card.alt} className="hero__avatar" />
                  </div>
                ) : (
          ...
          <svg className="hero__svg-lines" viewBox="0 0 1000 300" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <path d="M 370 150 L 260 120 L 110 120" className={`hero__svg-glow ${highlightedCardId === 'avatar-left' ? 'hero__svg-glow--active' : ''}`} stroke="#a78bfa" strokeWidth="2.5" />
            <path d="M 370 150 L 260 120 L 235 60" className={`hero__svg-glow ${highlightedCardId === 'idea' ? 'hero__svg-glow--active' : ''}`} stroke="#ffe86c" strokeWidth="2.5" />
            <path d="M 370 150 L 260 120 L 250 205" className={`hero__svg-glow ${highlightedCardId === 'team' ? 'hero__svg-glow--active' : ''}`} stroke="#49c4ff" strokeWidth="2.5" />
            <path d="M 470 150 L 580 110 L 870 110" className={`hero__svg-glow ${highlightedCardId === 'eye' ? 'hero__svg-glow--active' : ''}`} stroke="#cbd5e1" strokeWidth="2.5" />
            <path d="M 470 150 L 580 110 L 645 55" className={`hero__svg-glow ${highlightedCardId === 'security' ? 'hero__svg-glow--active' : ''}`} stroke="#ff6037" strokeWidth="2.5" />
            <path d="M 470 150 L 580 110 L 665 190" className={`hero__svg-glow ${highlightedCardId === 'avatar-right' ? 'hero__svg-glow--active' : ''}`} stroke="#ff8b6c" strokeWidth="2.5" />

            <path d="M 370 150 L 260 120 L 110 120" className={`hero__svg-pulse ${highlightedCardId === 'avatar-left' ? 'hero__svg-pulse--active' : ''}`} stroke="#a78bfa" strokeWidth="2.5" />
            <path d="M 370 150 L 260 120 L 235 60" className={`hero__svg-pulse ${highlightedCardId === 'idea' ? 'hero__svg-pulse--active' : ''}`} stroke="#ffe86c" strokeWidth="2.5" />
            <path d="M 370 150 L 260 120 L 250 205" className={`hero__svg-pulse ${highlightedCardId === 'team' ? 'hero__svg-pulse--active' : ''}`} stroke="#49c4ff" strokeWidth="2.5" />
            <path d="M 470 150 L 580 110 L 870 110" className={`hero__svg-pulse ${highlightedCardId === 'eye' ? 'hero__svg-pulse--active' : ''}`} stroke="#cbd5e1" strokeWidth="2.5" />
            <path d="M 470 150 L 580 110 L 645 55" className={`hero__svg-pulse ${highlightedCardId === 'security' ? 'hero__svg-pulse--active' : ''}`} stroke="#ff6037" strokeWidth="2.5" />
            <path d="M 470 150 L 580 110 L 665 190" className={`hero__svg-pulse ${highlightedCardId === 'avatar-right' ? 'hero__svg-pulse--active' : ''}`} stroke="#ff8b6c" strokeWidth="2.5" />
        ...
        <div className="hero__content">
          <h1 className="hero__title">All-in-one HR Platform</h1>
          <p className="hero__description">CoreShift is a modern, all-in-one HR platform designed to perfectly fit your business needs.</p>
